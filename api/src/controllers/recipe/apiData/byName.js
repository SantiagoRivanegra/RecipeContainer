...
const byName = async(name) => {
  const byName = await axios.get(`https://www.themealdb.com/api/json/v1/1/search.php?s=${name}`)
      if(byName.data.meals){
      const recipeName = await byName.data.meals.map(r => {
        return {
          id: r.idMeal,
